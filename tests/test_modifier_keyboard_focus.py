...
def test_modifier_keyboard_focus(mobile_page):
    ...
    is_textarea_focused, active_id = mobile_page.evaluate(f'''() => {{
        const activeElement = document.activeElement;
        const textarea = document.getElementById("terminal-input-{active_tab_id}");
        return [activeElement === textarea, activeElement ? activeElement.id || activeElement.tagName : "none"];
    }}''')
    
    assert is_textarea_focused is True, f"The terminal hidden textarea was not immediately focused upon tapping a modifier button. Instead focused: {active_id}"
    ...
    is_textarea_focused_alt = mobile_page.evaluate(f'''() => {{
        const activeElement = document.activeElement;
        const textarea = document.getElementById("terminal-input-{active_tab_id}");
