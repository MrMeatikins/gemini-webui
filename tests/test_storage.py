...
def test_init_app_full(test_data_dir):
    ...
    with patch('src.app.os.makedirs') as mock_mkdir, \
         patch('src.app.os.chmod') as mock_chmod, \
...
def test_ssh_key_rotation_logic(test_data_dir):
    ...
    with open(key_path, "w") as f: f.write("old key")
    with open(key_path + ".pub", "w") as f: f.write("old public key")
    
    with patch('src.app.subprocess.run') as mock_run:
        def side_effect(*args, **kwargs):
            # Create the files that the code expects to be generated by ssh-keygen
            with open(key_path, "w") as f: f.write("new key")
            with open(key_path + ".pub", "w") as f: f.write("new public key")
