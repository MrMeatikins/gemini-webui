...
def test_csrf_protection_missing_token(client):
    ...
    assert b'"csrf_expired":true' in response.data or b'"csrf_expired": true' in response.data
...
def test_csrf_ui_hard_reload(csrf_enabled_server_csrf):
    with sync_playwright() as p:
        ...
        expect(page.locator('.tab-instance.active .terminal-instance').first).to_be_visible(timeout=8000)
