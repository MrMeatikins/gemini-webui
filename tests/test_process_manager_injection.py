...
def test_build_terminal_command_resume_injection_local():
    ...
    cmd = build_terminal_command(None, None, "123; echo 'vulnerable'", "/tmp/.ssh", "gemini")
...
def test_build_terminal_command_resume_injection_ssh():
    ...
    cmd = build_terminal_command("user@host", "/tmp/dir", "123; echo 'vulnerable'", "/tmp/.ssh", "gemini")
...
def test_build_terminal_command_ssh_dir_injection_ssh():
    ...
    cmd = build_terminal_command("user@host", "my_dir; rm -rf /", "123", "/tmp/.ssh", "gemini")
...
def test_build_terminal_command_gemini_bin_injection_ssh():
    import shlex
    cmd = build_terminal_command("user@host", "/tmp/dir", "123", "/tmp/.ssh", "gemini; rm -rf /")
    remote_cmd = cmd[-1]
