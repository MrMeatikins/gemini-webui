...
def ssh_target_container(test_data_dir):
    ...
    if not os.path.exists(key_path):
        subprocess.run(["ssh-keygen", "-t", "ed25519", "-N", "", "-f", key_path, "-C", "test-key"], check=True)
    ...
    for _ in range(30):
        try:
            result = subprocess.run(["docker", "exec", container_name, "bash", "-c", "echo ready"], capture_output=True, text=True)
    ...
    subprocess.run(["docker", "exec", container_name, "apk", "add", "--no-cache", "python3", "bash"], check=True)
    
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    src_path = os.path.join(project_root, "src")
    
    # Put src dir at /app/src
    subprocess.run(["docker", "exec", container_name, "mkdir", "-p", "/app"], check=True)
    ...
    subprocess.run(["docker", "exec", container_name, "bash", "-c", f"echo -e '{script}' > /usr/local/bin/gemini && chmod +x /usr/local/bin/gemini"], check=True)
...
def test_ssh_drag_and_drop_upload(page, test_data_dir, ssh_target_container):
    ...
    for _ in range(10):
        ...
        result = subprocess.run(
            ["docker", "exec", container_name, "cat", "/config/remote_workspace/e2e_ssh_upload.txt"], 
        ...
        pytest.fail(f"File not found in SSH container or content mismatch. Last output: {result.stderr}")
