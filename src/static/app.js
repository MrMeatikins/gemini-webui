            ...
            if (stored) {
                try {
                    ...
                    data.forEach(t => {
                        ...
                        container.id = tab.id + '_instance'; container.className = 'tab-instance';
                    ...
                    return true;
                } catch (e) { console.error("Restore failed", e); sessionStorage.clear(); }
            }
            return false;
        }
                        ...
                        html += `<div class="session-item" style="align-items: center;">
                            <div style="flex-grow: 1; margin-right: 10px; min-width: 0;">
                                <div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: #999;">
                                    ${dirContext}<span class="session-name" style="color: #3b8eea; font-weight: bold;">${s.name}</span>
                                </div>
                                <div style="font-size: 10px; opacity: 0.7; color: #999; margin-top: 2px;">ID #${s.id} • ${s.meta}</div>
                            </div>
                            <div style="flex-shrink: 0;">
                                <button class="small" onclick="startSession('${tabId}', '${conn.type}', '${conn.target || ''}', '${conn.dir || ''}', '${s.id}', '${s.name.replace(/'/g, "\\'")}')">Resume</button>
                            </div>
                        </div>`;
                    });
        ...
        if (mobileControlsContainer) {
            mobileControlsContainer.addEventListener('touchmove', (e) => {
                e.preventDefault();
            }, { passive: false });
        }
        ...
        document.querySelectorAll('.control-btn.holdable').forEach(btn => {
            ...
            const executeAction = (e) => {
                ...
                if (cmd) {
                    ...
                    cmd = cmd.replace(/\\x([0-9A-Fa-f]{2})/g, (match, hex) => String.fromCharCode(parseInt(hex, 16)));
                    cmd = cmd.replace(/\\t/g, '\t');
